...
def exponential_mechanism(
    ...
    gamma: float = 1.0,        # New: RBF bandwidth
    delta_u: float = 1.0,      # Sensitivity for RBF is 1.0
    ...
    # 1. Compute Cosine Similarities first (efficiently)
    chunk = 50_000
    cosines = np.empty(N, dtype=np.float32)
    for start in range(0, N, chunk):
        end = min(start + chunk, N)
        cosines[start:end] = (corpus_embs[start:end] @ e_cpu).numpy()

    # 2. Transform Cosines to RBF Utility: exp(-2 * gamma * (1 - cos))
    # We apply this AFTER filtering for efficiency, or to all if needed.
    # Note: Percentile filtering on cosines is identical to filtering on RBF.
    if top_percentile < 1.0:
        cutoff = float(np.percentile(cosines, 100.0 * (1.0 - top_percentile)))
        mask = cosines >= cutoff
    else:
        mask = np.ones(N, dtype=bool)

    filtered_idx = np.where(mask)[0]
    filtered_cosines = cosines[filtered_idx]
    
    # Calculate RBF Utility
    utilities = np.exp(-2.0 * gamma * (1.0 - filtered_cosines))
    
    K = len(filtered_idx)
    log.info(
        f"[EM-RBF] gamma={gamma} | t={top_percentile:.2f} => {K:,}/{N:,} candidates "
        f"(u_rbf in [{utilities.min():.4f}, {utilities.max():.4f}])"
    )

    # 3. Gumbel-max trick
    # Sensitivity for RBF is 1.0 because the range is (0, 1]
