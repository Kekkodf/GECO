...
def exponential_mechanism(
    ...
    """
    Select one MS MARCO query via the exponential mechanism over the
    top-t% of the corpus ranked by cosine utility.

    DP validity of the percentile filter
    -------------------------------------
    Retaining only the top-(t*100)% of candidates by utility is a
    deterministic truncation of the mechanism's support.  This can only
    *tighten* the epsilon-d_X-DP guarantee: the proof relies on bounding
    the ratio P(output=c|x) / P(output=c|x') <= exp(eps * d(x,x')), which
    remains valid when the support is restricted to a subset.

    Global sensitivity
    ------------------
    u(x, q_k) = phi(x) . phi(q_k)  (cosine sim on unit sphere)
    Delta_u = max_{x,x'} |u(x,q) - u(x',q)|
            = max |(e - e') . e_k|
           <= ||e - e'||_2  (Cauchy-Schwarz, ||e_k||=1)
           <= 2              (diameter of S^{d-1})

    Parameters
    ----------
    e               : (d,) unit embedding of input x
    corpus_embs     : (N, d) unit embeddings of MS MARCO corpus (CPU tensor)
    corpus_queries  : N raw query strings
    epsilon         : DP privacy budget
    top_percentile  : t in (0, 1] -- fraction of corpus retained before selection
    delta_u         : global sensitivity (default 2.0)
    seed            : optional RNG seed

    Returns
    -------
    (selected_query, selected_embedding, probs, utilities)
    selected_query     : chosen MS MARCO query text
    selected_embedding : (d,) tensor -- real on-manifold embedding phi(q*)
    probs              : (K_filtered,) true EM probabilities (diagnostics)
    utilities          : (K_filtered,) cosine similarities after filtering
    """
    ...
    if seed is not None:
        np.random.seed(seed) #TODO: Set it in the init
    ...
    for start in range(0, N, chunk):
        ...
        all_utils[start:end] = (corpus_embs[start:end] @ e_cpu).numpy()
    ...
    K            = len(filtered_idx)
    log.info(
        f"[EM] t={top_percentile:.2f} => {K:,}/{N:,} candidates "
        f"(u in [{utilities.min():.4f}, {utilities.max():.4f}])"
    )

    # Gumbel-max trick (O(K), equivalent to sampling from EM distribution)
    ...
    # True EM probabilities for diagnostics (not used in sampling)
...
class Vec2TextDPObfuscator:
    ...
    def obfuscate(
        ...
        """Obfuscate `text` with privacy budget epsilon."""
        ...
        # Phase 2+3: filter + exponential mechanism
...
def main() -> None:
    ...
    obfuscator = Vec2TextDPObfuscator(
        ...
        cache_path="./data/query_pool/msmarco_gtr_embeddings.pt",
        max_queries=None,
    )
