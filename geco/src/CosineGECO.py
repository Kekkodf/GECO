...
class CosineGECO(AbstractGECO.Obfuscator):
    ...
    -------------------------
    Extends the AbstractGECO Obfuscator class and implements cosine similarity as the utility function for measuring the similarity between the query embedding and the pool embeddings.
    
    The global sensitivity is set to 2.0 for the cosine similarity. 
    ...
    The CosineGECO class inherits from the AbstractGECO Obfuscator class and provides methods for obfuscating a single text or a batch of texts, and it also includes diagnostics such as cosine similarity between the input and output embeddings, and the top candidates from the exponential mechanism.
    """
    ...
    def __str__(self):
        return f"CosineGECO Obfuscator with st_model={self.st_model_name}, inversion_model={self.inversion_model_name}, corrector_model={self.corrector_model_name}, top_percentile={self.top_percentile}, n_corpus={self.n_pool}, epsilon_values={self.epsilons}, utility_function={self.utility_function}, global_sensitivity={self.global_sensitivity}."
