...
class Obfuscator:
    ...
    *GECO (GEnerating Contextually Obfuscated Texts)* is a text obfuscation mechanism that generates a privatised version of an input text by selecting a pin query from a pool of queries based on the exponential mechanism and then reconstructing a new text from the embedding of the selected query using a corrector model. The mechanism is designed to provide differential privacy guarantees while maintaining the utility of the obfuscated text.
    ...
    def __init__(self,
        ...
        **kwargs) -> None:
        ...
        WARNING: this is an abstract class and should not be instantiated directly. It is meant to be subclassed by specific implementations of the GECO mechanism that may use different models or configurations.
            ...
            :param strategy: the strategy to use for constructing the pool of queries for the exponential mechanism. This determines whether the pool will consist of the original queries from the collection ("external") or if it will consist of reconstructions of those queries generated by Vec2Text ("internal"). The "internal" strategy may provide stronger privacy guarantees but will have a higher initialization cost due to the need to reconstruct and re-embed all queries in the pool. Defaults to "external".
    ...
    def _load_or_build_pool_embeddings(self, queries: List[str]) -> Tuple[List[str], torch.Tensor]:
        """
        Build (or load from cache) the candidate pool. Dispatches on self.strategy:
          - 'external': pool == original queries from collection (current behaviour).
          - 'internal': pool == Vec2Text reconstructions of those queries, re-embedded.
        The internal pool is cached separately so the two never collide.
        """
        base = Path(self.cache_path_pins)
        if self.strategy == "internal":
            cache = base.with_name(f"{base.stem}_internal{base.suffix}")
        else:
            cache = base

        if cache.exists():
            self.log.info(f"Loading embedding cache from {cache}")
            data = torch.load(str(cache), map_location="cpu")
            cq: List[str] = data["queries"]
            ce: torch.Tensor = data["embeddings"]
            # For 'internal' the cached queries are reconstructions, not the originals,
            # but the count is still 1:1 with the source collection.
            if len(cq) == len(queries):
                self.log.info(f"Cache valid: {len(cq):,} entries x {ce.shape[1]} dim")
                return cq, ce
            self.log.error(
                f"Cache size mismatch ({len(cq):,} vs {len(queries):,}). Re-building."
            )

        self.log.info(f"Encoding {len(queries):,} source queries (batch={self.encode_batch_size}).")
        source_embeddings = self.st_model.encode(
            sentences=queries,
        ...
        if self.strategy == "external":
            torch.save({"queries": queries, "embeddings": source_embeddings}, str(cache))
            self.log.info(f"External pool cache saved to {cache}.")
            return queries, source_embeddings

        self.log.warning(
            f"Building INTERNAL pool: this reconstructs {len(queries):,} embeddings "
            f"through Vec2Text. Expect a one-off cost; result will be cached at {cache}."
        )
        recon_texts = self._reconstruct_pool_texts(source_embeddings)

        self.log.info(f"Re-encoding {len(recon_texts):,} reconstructed texts.")
        recon_embeddings = self.st_model.encode(
            sentences=recon_texts,
            batch_size=self.encode_batch_size,
            convert_to_tensor=True,
            normalize_embeddings=True,
            show_progress_bar=True,
        ...
        torch.save({"queries": recon_texts, "embeddings": recon_embeddings}, str(cache))
        self.log.info(f"Internal pool cache saved to {cache}.")
        return recon_texts, recon_embeddings


def _reconstruct_pool_texts(
    self,
    embeddings: torch.Tensor,
    num_corrector_steps: int = 1,
) -> List[str]:
    """
    Batched Vec2Text inversion over the full pool. Used only for strategy='internal'.
    Robust to list[str] / list[list[str]] return shapes from vec2text.
    """
    device = next(self.corrector.model.parameters()).device
    out: List[str] = []
    N = len(embeddings)
    bs = self.invert_batch_size
        ...
        for start in range(0, N, bs):
            ...
            for t in texts:
                while isinstance(t, list):
                    t = t[0] if t else ""
                out.append(t.strip() if isinstance(t, str) else str(t))

            if ((start // bs) % 20) == 0:
                self.log.info(f"  Vec2Text reconstruction: {end:,}/{N:,}")
    ...
    def generate_from_embedding(self,
        ...
        # vec2text may return List[str] or List[List[str]] depending on version/config.
        # Unwrap defensively until we hit a string.
        result = texts[0] if len(texts) > 0 else ""
        ...
        if not isinstance(result, str):
            self.log.warning(f"Unexpected vec2text output type: {type(result)}; coercing to str.")
            result = str(result)
