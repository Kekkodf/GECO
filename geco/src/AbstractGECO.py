...
class Obfuscator:
    ...
    *GECO (GEnerating Contextually Obfuscated Texts)* is a text obfuscation mechanism that generates a privatised version of an input text by selecting a pin query from a pool of queries based on the exponential mechanism and then reconstructing a new text from the embedding of the selected query using a corrector model. The mechanism is designed to provide differential privacy guarantees while maintaining the utility of the obfuscated text.
    ...
    def __init__(self,
        ...
        **kwargs) -> None:
        ...
        for key, value in kwargs.items():
            ...
            self.log.info(f"Parameter '{key}' set to: {value}")
    ...
    def obfuscate(self,
        ...
        debug_info = (selected_query, cos_io, cos_iq, angle, top_candidates)
        answer = (output_text, debug_info)
    ...
    def generate_from_embedding(self,
        ...
        # vec2text can return list[str] or list[list[str]] depending on
        # version/config — unwrap until we hit a string.
        result = texts[0]
        while isinstance(result, list):
            if not result:
                raise RuntimeError("vec2text returned an empty result for the target embedding.")
            result = result[0]

        if not isinstance(result, str):
            raise TypeError(
                f"Expected str from vec2text.invert_embeddings, got {type(result).__name__}: {result!r}"
            )
